/*
  ESP32-S3 Wireless Mouse — 5-Button Variant
  --------------------------------------------
  Relays a wired USB mouse (Left / Right / Middle / Back / Forward buttons
  + scroll wheel) to any BLE-paired host as a wireless mouse.

  Adds Back/Forward (side navigation) button support on top of the base
  3-button variant. Standard 5-button mice report these in the same
  buttons byte as Left/Right/Middle:
    bit 0 (0x01) = Left
    bit 1 (0x02) = Right
    bit 2 (0x04) = Middle
    bit 3 (0x08) = Back / Previous
    bit 4 (0x10) = Forward / Next

  If your mouse's side buttons don't respond, or respond swapped, add a
  temporary debug line to print event.buttons over a UART/FTDI adapter
  (see README) while clicking each side button alone to find the real
  bit positions for your specific mouse.
...
void setup() {
  ...
  usbHost.onMouse([](const EspUsbHostMouseEvent &event) {
    ...
    if (event.buttonsChanged) {
      ...
      bleMouse.setButton(MouseButton::Back,    event.buttons & 0x08);
      bleMouse.setButton(MouseButton::Forward, event.buttons & 0x10);
      if (event.buttons != 0) {
